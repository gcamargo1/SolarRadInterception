...
def rad_intercpt_cycles(crop_list):
    ...
    for i in range(number_species):
        ...
        hght_wght_fct_adj[i] = (hght_wght_fct[i] /
                                     k_lai_prod_adj.sum() * k_lai_prod.sum())
...
def rad_intercpt_wallace(crop_list):
    ...
    assert len(crop_list[0]) == 3, "Only 3 inputs per species: ext_coeff,\
                                    LAI, height"
    assert len(crop_list[1]) == 3, "Only 3 inputs per species: ext_coeff, \
                                    LAI, height"
    # Read inputs
...
def rad_intercpt_apsim(crop_list):
    ...
    Reference:Carberry, P.S., Adiku, S.G.K., McCown, R.L., Keating, B.A.,1996.
     Application of the APSIM cropping systems model to intercropping systems,
     in: Ito, C., Johansen, C., Adu-Gyamfi, K., Katayama, K., Kumar-Rao,
     J.V.D.K., Rego, T.J. (Eds.), Dynamics of roots and nitrogen in cropping
     systems of the semi-arid tropics. Japan Int. Res. Centre Agric. Sci,
     pp. 637-648.
