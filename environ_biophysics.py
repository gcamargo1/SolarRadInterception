...
def opt_air_mass(atm_press: float, solar_zenith_angle: float) -> float:
...
def rad_ext_coeff_black_beam(solar_zenith_angle: float, x_area_ratio: float) -> float:
...
def rad_ext_coeff_black_diff(x_area_ratio: float, leaf_area_index: float) -> float:
    ...
    while True:
        ...
        transm_beam = math.exp(
            -rad_ext_coeff_black_beam(angle, x_area_ratio) * leaf_area_index
        )
            ...
            sp1_intercpt_alone[i, j] = (
                ...
                - (
                    ...
                    * math.exp(
                        -(leaf_transm**0.5)
                        * lai
                        * rad_ext_coeff_black_diff(x_sp1, lai)
                    )
                )
            ) * total_intercpt[i]
            sp2_intercpt_alone[i, j] = (
                ...
                - (
                    ...
                    * math.exp(
                        -(leaf_transm**0.5)
                        * lai
                        * rad_ext_coeff_black_diff(x_sp2, lai)
                    )
                )
            ) * total_intercpt[i]
            canopy_intercpt[i, j] = (
